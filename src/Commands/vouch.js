...
const builder = new SlashCommandBuilder()
	...
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			...
			.addIntegerOption((opt) =>
				opt
					.setName('cooldown')
					.setDescription('The cooldown to be applied to make vouches (Minutes).')
					.setMinValue(2)
					.setMaxValue(15)
					.setRequired(true)
			)
	...
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			.setName('give')
			.setDescription('Vouches someone, adding +1 to their vouch score in the current guild.')
			.addUserOption((opt) => opt.setName('user').setDescription('The user to vouch').setRequired(true))
			.addStringOption((opt) => opt.setName('reason').setDescription("The reason you're vouching this user for"))
	)
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			.setName('check')
			...
			.addUserOption((opt) =>
				opt
					.setName('user')
					.setDescription('The user to check the vouches of, if left blank your own vouches will be shown.')
			)
	)
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			...
			.addUserOption((opt) =>
				opt.setName('user').setDescription('The user to set the vouches of.').setRequired(true)
			)
			.addIntegerOption((opt) =>
				opt.setName('amount').setMinValue(0).setDescription('The vouches to set to this user').setRequired(true)
			)
			.addStringOption((opt) =>
				opt.setName('reason').setDescription("The reason you're setting the vouches of this user")
			)
	);
...
const onInteraction = async ({ int, client }) => {
	...
	if ((await guildManager.find({ guildId: int.guild.id }).exec())[0] === undefined) {
		await guildManager.create({ guildId: int.guild.id });
	}
	const { vouchSettings } = (await guildManager.find({ guildId: int.guild.id }).exec())[0];
	...
	if (int.options.getSubcommand() === 'config') {
		...
		const cd = typeof vouchSettings.cooldown === 'number' ? vouchSettings.cooldown : 180000;
		if (vouches.find((v) => v.userId === int.user.id).lastVouch + cd > Date.now())
			return int.reply({
				ephemeral: true,
				content: `You have already vouched someone in the last ${cd / 6e4} minutes!`
			});
		...
		const user = int.options.getUser('user') !== null ? int.options.getUser('user') : int.user;
		...
		await int.reply({
			embeds: [
				new MessageEmbed()
					...
					.setTimestamp()
					.setThumbnail(user.avatarURL() !== null ? user.avatarURL() : client.avatarURL())
