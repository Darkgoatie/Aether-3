    ...
    switch (subcommand) {
        case "start":
            if (!int.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) {
                return replyEphemeral("You don't have the manage messages permissions required for this command!");
            }

            channel = int.options.getChannel("channel") || int.channel;
            auc = await findActiveAuction(channel.id);
            if (auc) {
                return replyEphemeral("There is already an active auction in this channel!");
            }
        ...
        case "bid":
            ...
            auc = (await auctionManager.find({channelId: int.channel.id}).exec())[0];
            if (auc === undefined) return int.reply({
                ephemeral: true, content: "Couldn't find any active auctions in this channel!",
            });
            bid = int.options.getInteger("amount");
            if (bid <= auc.price) return int.reply({
                ephemeral: true,
            ...
            await auctionManager.updateOne({channelId: int.channel.id}, {
            ...
            if (isAnonymous === false) {
                await int.reply({
                    embeds: [new MessageEmbed()
                        ...
                        .addField("Amount", `${bid}`),], components: [row],
                });
            } else if (isAnonymous === true) {
                await int.reply({
                    ephemeral: true, content: "Your bid was anonymously created!",
                });
        ...
        case "end":
            channel = int.options.getChannel("channel") !== null ? int.options.getChannel("channel") : int.channel;
            auc = (await auctionManager.find({channelId: channel.id}).exec())[0];
            if (auc === undefined) return int.reply({
                ephemeral: true, content: "Couldn't find any active auctions in this channel!",
            });
            if (auc.hostedBy !== int.user.id && !int.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return int.reply({
                ephemeral: true,
                content: "You should either have started the auction, or should have manage guild permissions to end it!",
            });
            sent = await channel.send({
                embeds: [new MessageEmbed()
            ...
            client.users.fetch(auc.winner).then((usr) => usr.send({
        ...
        case "autoend":
            ...
            if (typeof time != "number") return int.reply({
                ephemeral: true, content: "Not a valid amt of time!",
            });
            if (time > ms("1w")) return int.reply({
                ephemeral: true, content: "Time can't be longer than a week!",
            });
            auc = (await auctionManager.find({channelId: int.channel.id}).exec())[0];
            if (auc === undefined) return int.reply({
                ephemeral: true, content: "Couldn't find any active auctions in this channel!",
            ...
            await int.reply({
                content: "A timer was started!", ephemeral: true,
            });
            ...
            await auctionManager.updateOne({
                ...
                autoEndSettings: {
                    autoEnd: true, endAt: Date.now() + time, timer: {
                        messageId: smsg.id,
                    },
                },
            });
        ...
        case "setprice":
            ...
            if (auc.hostedBy !== int.user.id && !int.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return int.reply({
                ephemeral: true,
                content: "You should either have started the auction, or should have manage guild permissions to edit it!",
            });
