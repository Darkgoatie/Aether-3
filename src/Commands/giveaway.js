...
const builder = new SlashCommandBuilder()
	...
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			...
			.addStringOption((opt) =>
				opt.setName('time').setDescription('The time the giveaway will end in.').setRequired(true)
			)
			.addStringOption((opt) =>
				opt.setName('prize').setDescription('The Prize of the giveaway').setRequired(true)
			)
	...
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			...
			.addStringOption((opt) =>
				opt.setName('id').setDescription('The ID of the giveaway that will be ended').setRequired(true)
			)
	)
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			...
			.addStringOption((opt) =>
				opt.setName('id').setDescription('The ID of the giveaway that will be rerolled').setRequired(true)
			)
	)
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			...
			.addBooleanOption((opt) =>
				opt.setName('ended').setDescription('Filter to see the giveaways that are ended or not.')
			)
	)
	.addSubcommand(
		new SlashCommandSubcommandBuilder()
			...
			.addStringOption((opt) =>
				opt.setName('emoji').setDescription("The name of the emoji that'll be set as default").setRequired(true)
			)
	);
...
const onInteraction = async ({ int, client }) => {
	...
	if (int.options.getSubcommand() === 'start') {
		...
		if (
			...
			typeof int.guild.emojis.find((e) => e.id === thisGuildConf.giveawaySettings.emoji) !== 'undefined'
		) {
		...
		int.reply({
			content: `A giveaway has been successfully started in <#${giveawayChannel
				? giveawayChannel.id
				: int.channel.id}> !`,
			ephemeral: true
		});
		...
		for (let embedCount = 0; embedCount < 10 && gws.length > 0; embedCount++) {
			const emb = new MessageEmbed().setTitle('Giveaways list').setFooter({ text: `Page ${embedCount}` });
