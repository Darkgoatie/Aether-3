...
const onInteraction = async ({int, client}) => {
    ...
    switch (subcommand) {
        case "start":
            ...
            let tgEmoji = {
                id: "866637037607845929", name: "AetherGift",
            };
            ...
            if (typeof thisGuildConf.giveawaySettings.emoji !== undefined || thisGuildConf.giveawaySettings.emoji === null || typeof int.guild.emojis.find((e) => e.id === thisGuildConf.giveawaySettings.emoji) !== "undefined") {
                Cemoji = int.guild.emojis.cache.find((e) => e.id === thisGuildConf.giveawaySettings.emoji);
                tgEmoji = int.guild.emojis.cache.find((e) => e.id === thisGuildConf.giveawaySettings.emoji);
            }
            if (!int.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) return int.reply({
                ephemeral: true, content: "You need manage messages permissions to use this command!",
            });
            ...
            if (isNaN(gwLength)) return int.reply({
                content: "Inputted time is not a valid amount of time! Try using these: \n 1d, 1d12h, 30m, 6h",
                ephemeral: true,
            });
            if (gwLength > parseInt(ms("7d"))) return int.reply({
                content: "This time is too long! Maximum length of a giveaway shall be a week (7 days)!",
                ephemeral: true,
            });
            await client.giveawaysManager.start(giveawayChannel ? giveawayChannel : int.channel, {
                duration: gwLength, winnerCount: winners, prize, messages: {
                ...
                }, reaction: Cemoji, extraData: {
                    hostedBy: int.user.id,
                },
            });
            int.reply({
                content: `A giveaway has been successfully started in <#${giveawayChannel ? giveawayChannel.id : int.channel.id}> !`,
                ephemeral: true,
            });
        ...
        case "end":
            ...
            client.giveawaysManager
                ...
                .then(() => {
                    int.reply({
                        ephemeral: true, content: `Giveaway with id "${messageId}" was ended!`,
                    });
                })
                .catch((err) => {
        ...
        case "reroll":
            ...
            client.giveawaysManager
                ...
                .then(() => {
                    int.reply({
                        ephemeral: true, content: `Giveaway with id "${messageId}" was rerolled!`,
                    });
                })
                .catch((err) => {
                    int.reply({ephemeral: true, content: `This giveaway was not found!`});
        ...
        case "list":
            ...
            gws = gws.map((gw) => {
                n++;
                return {
                    val: `Channel: <#${gw.channelId}>, Prize: ${gw.prize} `, nm: `Giveaway #${n}`,
                };
            });
        ...
        case "setemoji":
            if (!int.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return int.reply({
                ephemeral: true, content: "You need manage guild permissions to use this command!",
            });
            const emojiName = int.options.getString("emoji");
            const emoji = int.guild.emojis.cache.find((emj) => emj.name === emojiName);
            if (typeof emoji === "undefined" || emoji === null) return int.reply({
                ephemeral: true, content: `Emoji was not found! Query: { name: ${emojiName} }`,
            });
        ...
        default:
            ...
            int.reply({
                ephemeral: true, content: "Unknown subcommand!",
            });
    }
};

module.exports = {
    name, description, builder, onInteraction,
};